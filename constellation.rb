class Constellation
  ...
  def initialize(data)
    if data.kind_of?(Array)
      coordlist = data
    else data.kind_of?(String)
      coordlist = Constellation.parse_coordinates(data);
    end
  ...
  def self.precess(ra1, dec1, epoch1, epoch2)
    cdr = Math::PI / 180.0
    csr = cdr / 3600.0
    ...
    x2 = [
      (r[0][0]*x1[0] + r[0][1]*x1[1] + r[0][2]*x1[2]),
      (r[1][0]*x1[0] + r[1][1]*x1[1] + r[1][2]*x1[2]),
      (r[2][0]*x1[0] + r[2][1]*x1[1] + r[2][2]*x1[2])
    ]
  ...
  def self.get_name(ra, dec, epoch)
    convh = Math::PI / 12.0
    convd = Math::PI / 180.0
    ra *= convh
    dec *= convd
    newcoords = Constellation.precess(ra, dec, epoch, 1875.0)
    ra = newcoords[0]
    dec = newcoords[1]
    ra /= convh
    dec /= convd
    ...
    i = 0
    while i < self.boundries.length do
      if (dec < self.boundries[i][2] || ra < self.boundries[i][0] || ra >= self.boundries[i][1])
         i += 1
      else
        return self.boundries[i][3]
      end
    end
  ...
  def self.parse_coordinates(str)
    rah = 0.0
    ram = 0.0
    ras = 0.0
    decd = 0.0
    ...
    decd = exploded[nra].to_f
    
    if (ndec >= 2)
       decm = exploded[nra + 1].to_f
    end
    
    if (ndec >= 3)
       decs = exploded[nra + 2].to_f
    end
  
    unless rah.is_a?(Numeric) || ram.is_a?(Numeric) || ras.is_a?(Numeric) || decd.is_a?(Numeric) || decm.is_a?(Numeric) || decs.is_a?(Numeric)
       return false
    end
    ...
    if (decd < 0.0 || (decd == 0.0 && exploded[nra].length > 0 && exploded[nra][0] == '-'))
      decm = -decm;
      decs = -decs;
    end
